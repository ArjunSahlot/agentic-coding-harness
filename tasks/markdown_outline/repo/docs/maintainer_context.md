...
- Estimated context target: 12200 tokens
...
## Historical Scenarios
        ...
        ### Scenario 1: slug compatibility

        The static site host links directly to GitHub-compatible anchors. Punctuation removal, whitespace collapse, and duplicate suffixes must match the documented behavior closely enough for existing links to keep working.
        ...
        ### Scenario 2: example code

        Documentation authors often include markdown snippets showing intentionally broken links. Fenced code and inline code are instructional examples and should not become link-checker findings.
        ...
        ### Scenario 3: reference definitions

        Long pages prefer reference-style links so repeated URLs are easier to maintain. Definitions are case-insensitive and should resolve before relative path normalization.
        ...
        ### Scenario 4: fragment normalization

        Human-written fragments use spaces and punctuation. The indexer should normalize fragment ids using slug rules, but it should not apply duplicate suffixes across target documents it has not parsed.
        ...
        ### Scenario 5: absolute and external targets

        HTTP, HTTPS, mailto, and root-relative links are already absolute in their own namespace. Only document-relative links should be joined to the current file directory.
...
requiring package downloads or internet access.
        ...
        ### Context Slice 1: slug compatibility

        Background signal: The static site host links directly to GitHub-compatible anchors. Punctuation removal, whitespace collapse, and duplicate suffixes must match the documented behavior closely enough for existing links to keep working. In previous reviews, this kind
        ...
        `slug compatibility` without growing unrelated code paths.
        ...
        ### Context Slice 2: example code

        Background signal: Documentation authors often include markdown snippets showing intentionally broken links. Fenced code and inline code are instructional examples and should not become link-checker findings. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `example code` without growing unrelated code paths.
        ...
        ### Context Slice 3: reference definitions

        Background signal: Long pages prefer reference-style links so repeated URLs are easier to maintain. Definitions are case-insensitive and should resolve before relative path normalization. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `reference definitions` without growing unrelated code paths.
        ...
        ### Context Slice 4: fragment normalization

        Background signal: Human-written fragments use spaces and punctuation. The indexer should normalize fragment ids using slug rules, but it should not apply duplicate suffixes across target documents it has not parsed. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `fragment normalization` without growing unrelated code paths.
        ...
        ### Context Slice 5: absolute and external targets

        Background signal: HTTP, HTTPS, mailto, and root-relative links are already absolute in their own namespace. Only document-relative links should be joined to the current file directory. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `absolute and external targets` without growing unrelated code paths.
