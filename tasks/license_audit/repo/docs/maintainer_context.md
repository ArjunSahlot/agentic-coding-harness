...
- Estimated context target: 11600 tokens
...
## Historical Scenarios
        ...
        ### Scenario 1: transitive graph

        A direct dependency can pull in a problematic transitive package. Auditing only the root package misses the cases policy reviewers care about.
        ...
        Package metadata often uses human aliases such as Apache 2 or BSD 3-Clause. Normalize common aliases before comparing against SPDX policy names.
        ...
        ### Scenario 3: OR expressions

        A package licensed as MIT OR GPL-3.0 can be consumed under MIT if MIT is allowed. The audit should not fail just because one alternative is denied.
        ...
        ### Scenario 4: AND expressions

        A conjunctive expression requires satisfying every part. If any part is denied, the package is a violation with reason denied.
        ...
        ### Scenario 5: package exceptions

        Legal approvals are package-specific. An exception for one LGPL package should not broadly allow LGPL across the graph.
...
requiring package downloads or internet access.
        ...
        ### Context Slice 1: transitive graph

        Background signal: A direct dependency can pull in a problematic transitive package. Auditing only the root package misses the cases policy reviewers care about. In previous reviews, this kind
        ...
        `transitive graph` without growing unrelated code paths.
        ...
        ### Context Slice 2: alias normalization

        Background signal: Package metadata often uses human aliases such as Apache 2 or BSD 3-Clause. Normalize common aliases before comparing against SPDX policy names. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `alias normalization` without growing unrelated code paths.
        ...
        ### Context Slice 3: OR expressions

        Background signal: A package licensed as MIT OR GPL-3.0 can be consumed under MIT if MIT is allowed. The audit should not fail just because one alternative is denied. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `OR expressions` without growing unrelated code paths.
        ...
        ### Context Slice 4: AND expressions

        Background signal: A conjunctive expression requires satisfying every part. If any part is denied, the package is a violation with reason denied. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `AND expressions` without growing unrelated code paths.
        ...
        ### Context Slice 5: package exceptions

        Background signal: Legal approvals are package-specific. An exception for one LGPL package should not broadly allow LGPL across the graph. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `package exceptions` without growing unrelated code paths.
