...
- Estimated context target: 12100 tokens
...
## Historical Scenarios
        ...
        ### Scenario 1: environment layering

        Staging and production can have different defaults and rules. The evaluator should read the requested environment and fall back deliberately rather than ignoring environment blocks.
        ...
        Python's hash is process-randomized and random seeding has cross-flag collisions. Rollout buckets must use the specified SHA-256 flag/user key hash.
        ...
        ### Scenario 3: rule matching

        Rules may target user segments or attribute equality. The first matching rule provides a value, then explicit overrides can still replace it.
        ...
        ### Scenario 4: override precedence

        Support staff use overrides to force-enable or block individual users during incidents. Overrides are the final authority for a flag in an environment.
        ...
        ### Scenario 5: unknown flags

        An unknown flag should evaluate to false rather than raising, which keeps client code resilient during staged config rollouts.
...
requiring package downloads or internet access.
        ...
        ### Context Slice 1: environment layering

        Background signal: Staging and production can have different defaults and rules. The evaluator should read the requested environment and fall back deliberately rather than ignoring environment blocks. In previous reviews, this kind
        ...
        `environment layering` without growing unrelated code paths.
        ...
        ### Context Slice 2: stable rollout

        Background signal: Python's hash is process-randomized and random seeding has cross-flag collisions. Rollout buckets must use the specified SHA-256 flag/user key hash. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `stable rollout` without growing unrelated code paths.
        ...
        ### Context Slice 3: rule matching

        Background signal: Rules may target user segments or attribute equality. The first matching rule provides a value, then explicit overrides can still replace it. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `rule matching` without growing unrelated code paths.
        ...
        ### Context Slice 4: override precedence

        Background signal: Support staff use overrides to force-enable or block individual users during incidents. Overrides are the final authority for a flag in an environment. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `override precedence` without growing unrelated code paths.
        ...
        ### Context Slice 5: unknown flags

        Background signal: An unknown flag should evaluate to false rather than raising, which keeps client code resilient during staged config rollouts. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `unknown flags` without growing unrelated code paths.
