...
## Historical Scenarios
        ...
        ### Scenario 1: unordered delivery

        Event files are merged from shards and may arrive out of order. Replay must sort by timestamp and sequence before applying financial effects.
        ...
        ### Scenario 2: idempotent rebuild

        Retried ingestion can duplicate event ids. A duplicate id should be ignored, not rejected as an overdraft and not counted twice in balances.
        ...
        ### Scenario 3: opening snapshots

        Incremental rebuilds begin from a snapshot with balances and applied ids. The replay should not mutate caller-owned collections in surprising ways if it needs to derive a new state.
        ...
        ### Scenario 4: overdraft rejection

        Withdrawals and transfers cannot make source balances negative. Rejected events should include a reason and must not be added to applied ids.
        ...
        ### Scenario 5: decimal amounts

        Amounts are strings in event payloads. Decimal conversion keeps replay deterministic for currency values.
...
requiring package downloads or internet access.
        ...
        ### Context Slice 1: unordered delivery

        Background signal: Event files are merged from shards and may arrive out of order. Replay must sort by timestamp and sequence before applying financial effects. In previous reviews, this kind
        ...
        `unordered delivery` without growing unrelated code paths.
        ...
        ### Context Slice 2: idempotent rebuild

        Background signal: Retried ingestion can duplicate event ids. A duplicate id should be ignored, not rejected as an overdraft and not counted twice in balances. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `idempotent rebuild` without growing unrelated code paths.
        ...
        ### Context Slice 3: opening snapshots

        Background signal: Incremental rebuilds begin from a snapshot with balances and applied ids. The replay should not mutate caller-owned collections in surprising ways if it needs to derive a new state. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `opening snapshots` without growing unrelated code paths.
        ...
        ### Context Slice 4: overdraft rejection

        Background signal: Withdrawals and transfers cannot make source balances negative. Rejected events should include a reason and must not be added to applied ids. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `overdraft rejection` without growing unrelated code paths.
        ...
        ### Context Slice 5: decimal amounts

        Background signal: Amounts are strings in event payloads. Decimal conversion keeps replay deterministic for currency values. In previous reviews, this kind
        of scenario was easy to miss when the agent looked only at the first
        failing assertion. The useful context is split between the README,
        the implementation comments, and the unit tests. A context reduction
        strategy should keep the public API, the data shape, and the edge
        ...
        inconsistent. Prefer a small helper with a name that describes the
        rule. That gives future tests a clear place to exercise variants of
        `decimal amounts` without growing unrelated code paths.
